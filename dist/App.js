...
const App = () => {
  const [localBrews, setLocalBrews] = useState([]);
  ...
  function getUserLocation() {
    const success = (local) => getLocalBreweries(local.coords.latitude, local.coords.longitude);
  ...
  })), localBrews.filter((item) => filter === "All" ? item : item.brewery_type === filter.toLowerCase()).map((item) => /* @__PURE__ */ React.createElement("article", {
